...
static bool run_b2style(const std::string& content, const std::string& source_name)
{
    simulator sim;
    auto pw = b2style::with_default_functions();
    if (!pw.compile(content, sim))
    {
        std::cerr << "Error: Failed to compile b2style code from " << source_name << std::endl;
        return false;
    }
...
static bool run_file(const fs::path& p)
{
    ...
    {
        std::cerr << "Error: Failed to compile b2style file: " << p << std::endl;
        return false;
    }
...
int main(int argc, char* argv[])
{
    if (argc <= 1 || (argc == 2 && std::string(argv[1]) == "-"))
    {
        ...
        return run_b2style(content, "<stdin>") ? 0 : 2;
    }
