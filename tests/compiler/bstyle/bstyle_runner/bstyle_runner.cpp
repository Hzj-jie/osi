...
static bool run_bstyle(const std::string& content, const std::string& source_name)
{
    simulator sim;
    auto pw = bstyle::with_default_functions();
    if (!pw.compile(content, sim))
    {
        std::cerr << "Error: Failed to compile bstyle code from " << source_name << std::endl;
        return false;
    }
...
static bool run_file(const fs::path& p)
{
    ...
    {
        std::cerr << "Error: Failed to compile bstyle file: " << p << std::endl;
        return false;
    }
...
static bool run_path(const fs::path& p)
{
    if (fs::is_directory(p))
    {
        ...
        {
            if (entry.is_regular_file() && (entry.path().extension() == ".bstyle" || entry.path().extension() == ".txt"))
            {
...
int main(int argc, char* argv[])
{
    if (argc <= 1 || (argc == 2 && std::string(argv[1]) == "-"))
    {
        ...
        return run_bstyle(content, "<stdin>") ? 0 : 2;
    }
