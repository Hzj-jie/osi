...
static bool save_checkpoint(const std::string& path, uint64_t step,
...
static bool load_checkpoint(const std::string& path, uint64_t& step,
                            big_uint& N, big_uint& D_sum,
                            big_uint& T, big_uint& D_term,
                            bool& is_binary) {
    ...
    if (ifs.read(magic, 8) && std::memcmp(magic, "OSICKPT1", 8) == 0) {
        ...
        if (!N.read_binary(ifs) || !D_sum.read_binary(ifs) ||
            !T.read_binary(ifs) || !D_term.read_binary(ifs)) return false;
        is_binary = true;
        return true;
    }
    ifs.clear();
...
static int dump_to_decimal(const std::string& input_file, const std::string& output_file) {
    ...
    *os << step << "\n";
    if (!big_udec::binary_to_decimal(ifs, *os)) {
        std::cerr << "Error: Failed to deserialize sum (big_udec) from " << input_file << std::endl;
        return 1;
    }
    *os << "\n";
    if (!big_udec::binary_to_decimal(ifs, *os)) {
        std::cerr << "Error: Failed to deserialize term (big_udec) from " << input_file << std::endl;
        return 1;
    }
    *os << "\n";
    return 0;
...
int main(int argc, char* argv[]) {
    ...
    if (!load_target.empty()) {
        ...
        if (test_open.is_open()) {
            test_open.close();
            if (load_checkpoint(load_target, step, N, D_sum, T, D_term, is_binary)) {
                start_step = step + 1;
                if (is_binary) {
    ...
    for (uint64_t i = start_step; i <= max_iterations; ++i) {
        ...
        if (now - last_time >= interval_duration || i == max_iterations) {
            ...
            if (!checkpoint_file.empty()) {
                if (save_checkpoint(checkpoint_file, i, N, D_sum, T, D_term)) {
