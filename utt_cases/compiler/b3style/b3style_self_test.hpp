...
{
    class b3style_self_test : public icase
    {
    private:
        ...
        static fs::path find_directory(const std::string& name)
        {
            try
            {
                ...
                if (fs::exists(exe_path / "utt_cases/compiler/b3style" / name))
                {
                    return exe_path / "utt_cases/compiler/b3style" / name;
                }
                if (fs::exists(exe_path / "../utt_cases/compiler/b3style" / name))
                {
                    return exe_path / "../utt_cases/compiler/b3style" / name;
            ...
            if (fs::exists("utt_cases/compiler/b3style/" + name))
            {
                return fs::path("utt_cases/compiler/b3style/" + name);
            }
            ...
            if (fs::exists("tests/compiler/b3style/" + name))
            {
                return fs::path("tests/compiler/b3style/" + name);
            }
        ...
        static bool test_self_assertions(int& passed, int& failed, int& skipped)
        {
            ...
            std::cout << "Running b3style self-assertion tests (" << entries.size() << " files)...\n";
            ...
            for (const auto& path : entries)
            {
                ...
                auto guard = b3style::compile_wrapper::with_current_file(filename);
                bool compile_ok = b3style::with_functions(primitive::interrupts(io.io())).compile(content, sim);
                if (!compile_ok)
                {
        ...
        static bool test_self_compile_errors(int& passed, int& failed)
        {
            ...
            std::cout << "\nRunning b3style self-compile-error tests (" << entries.size() << " files)...\n";
            ...
            for (const auto& path : entries)
            {
                ...
                primitive::simulator sim;
                auto guard = b3style::compile_wrapper::with_current_file(filename);
                bool compile_ok = b3style::with_default_functions().compile(content, sim);
                if (compile_ok)
                {
    ...
    public:
        ...
        DEFINE_CASE(b3style_self_test);
    };

    REGISTER_CASE(b3style_self_test);
}
